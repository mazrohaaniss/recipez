...
const CreateRecipe = () => {
  ...
  const saveRecipe = (type) => {
    ...
    alert(type === "publish" ? "Recipe Berhasil Dipublish!" : "Recipe Berhasil Disimpan");
  ...
  const StorySection = () => (
    <textarea 
      placeholder="Latar Belakang Recipe (Optional)" 
      className="w-full p-3 mb-4 border border-gray-300 rounded-md" 
      value={story} 
    ...
    <div className="mb-6">
      ...
      {ingredients.map((ingredient, index) => (
        <div key={index} className="flex items-center space-x-2 mb-2">
          ...
          />
          <button onClick={() => deleteIngredient(index)} className="bg-red-500 text-white rounded-md p-2 hover:bg-red-600">
            <TrashIcon className="h-5 w-5" />
          </button>
        </div>
      ))}
      <button onClick={addIngredient} className="w-full p-3 bg-blue-600 text-white rounded-md font-semibold hover:bg-blue-700 mt-2">
        + Tambah Bahan
      </button>
    </div>
  );
    ...
    <div className="mb-6">
      ...
      {tools.map((tool, index) => (
        <div key={index} className="flex items-center space-x-2 mb-2">
          ...
          />
          <button onClick={() => deleteTool(index)} className="bg-red-500 text-white rounded-md p-2 hover:bg-red-600">
            <TrashIcon className="h-5 w-5" />
          </button>
        </div>
      ))}
      <button onClick={addTool} className="w-full p-3 bg-blue-600 text-white rounded-md font-semibold hover:bg-blue-700 mt-2">
        + Tambah Alat
      </button>
    </div>
  );
    ...
    <div className="mb-6">
      ...
      {steps.map((step, index) => (
        <div key={index} className="flex items-center space-x-2 mb-2">
          ...
          <button onClick={() => deleteStep(index)} className="bg-red-500 text-white rounded-md p-2 hover:bg-red-600">
            <TrashIcon className="h-5 w-5" />
          </button>
        </div>
      ))}
      <button onClick={addStep} className="w-full p-3 bg-blue-600 text-white rounded-md font-semibold hover:bg-blue-700 mt-2">
        + Tambah Langkah
      </button>
    </div>
  );
    ...
    <div className="bg-blue-50 min-h-screen p-8 flex flex-col items-center overflow-y-auto">
      <div className="w-full max-w-4xl bg-white p-6 rounded-lg shadow-lg">
        ...
        <div className="flex justify-between mt-8">
          <button onClick={() => saveRecipe('publish')} className="px-4 py-2 bg-orange-500 text-white rounded-lg font-semibold hover:bg-orange-600">
            Publish
          </button>
          <button onClick={() => saveRecipe('draft')} className="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg font-semibold hover:bg-gray-300">
            Save as Draft
          </button>
          <button onClick={clearForm} className="px-4 py-2 bg-red-500 text-white rounded-lg font-semibold hover:bg-red-600">
            Clear
          </button>
        </div>
      </div>
